        ...
        <div className="search">
            <div className="search__container container">
                ...
                <div className="search__body search-body">
                    ...
                    <div className="search-body__label">{currentModel ? currentModel : "Select Model"}</div>
                        <ul className={`search-menu ${modelPopupVisibility ? "opened" : "closed"}`}>
                            { props.models[currentBrand]
                            ? props.models[currentBrand].map(({model}) => <li key={model} className="search-menu__item" onClick={(event) => {changeCurrentModel(event.target.textContent)}}>{model}</li>)
                    ...
                    onClick={currentBrand && currentModel ? () => props.findCar(currentBrand, currentModel) : null}
                    >Search</button>
                    </>
                    }
